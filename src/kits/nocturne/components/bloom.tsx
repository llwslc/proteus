  ...
  return (
    <defs>
      ...
      <linearGradient id={`${id}-petal-lit`} x1="0" y1="1" x2="0" y2="0">
        <stop
          offset="0"
          style={{ stopColor: "var(--nocturne-bloom-tone, var(--nocturne-petal-lit))" }}
        />
        <stop
          offset="1"
          style={{
            stopColor: "var(--nocturne-bloom-tone-deep, var(--nocturne-petal-lit-deep))",
          }}
        />
      </linearGradient>
  ...
  return (
    <g transform={transform}>
        ...
        five.map((i) => (
          <g key={`s${i}`} transform={`rotate(${i * 72 + 36})`}>
            <path
              ...
              style={
                mode === "state" && sepalExpr
                  ? ({ "--nocturne-bloom-sepal": sepalExpr } as CSSProperties)
                  : undefined
              }
              d={sepalPath(r * 0.75)}
              fill={`url(#${defs}-leaf)`}
        ...
        <g key={`p${i}`} transform={`rotate(${i * 72})`}>
          <path
            ...
            style={
              mode === "entrance"
                ? ({
                    "--nocturne-d": `${(delay + i * 0.16).toFixed(2)}s`,
                  } as CSSProperties)
                : mode === "state" && openExpr
      ...
      <g
        ...
        style={
          mode === "entrance"
            ? ({ "--nocturne-d": `${(delay + 0.9).toFixed(2)}s` } as CSSProperties)
            : undefined
        }
      >
        <circle
          r={Number(cr.toFixed(2))}
          fill="var(--nocturne-gilt-bright)"
          stroke="var(--nocturne-gilt-dim)"
          strokeWidth={sw > 0.6 ? 1 : 0.5}
        />
        {coreDots === 5 ? (
          five.map((j) => {
          ...
          <g fill="var(--nocturne-primary-deep)">
            <circle
              cy={Number((-1.6 * k).toFixed(2))}
              r={Number((0.75 * k).toFixed(2))}
            />
            <circle
              cx={Number((1.5 * k).toFixed(2))}
              cy={Number((0.6 * k).toFixed(2))}
              r={Number((0.75 * k).toFixed(2))}
            />
            <circle
              cx={Number((-1.5 * k).toFixed(2))}
  ...
  return (
    ...
    >
      ...
      <Leaf
        ...
        style={{ "--nocturne-d": "0.8s" } as CSSProperties}
        transform="translate(155,21) rotate(-134) scale(.5)"
      />
      <Leaf
        defs={id}
        className="nocturne-sprout"
        style={{ "--nocturne-d": "0.8s" } as CSSProperties}
        transform="translate(245,21) rotate(134) scale(-.5,.5)"
      />
      <Leaf
        defs={id}
        className="nocturne-sprout"
        style={{ "--nocturne-d": "1.05s" } as CSSProperties}
        transform="translate(112,30) rotate(-166) scale(.44)"
      />
      <Leaf
        defs={id}
        className="nocturne-sprout"
        style={{ "--nocturne-d": "1.05s" } as CSSProperties}
        transform="translate(288,30) rotate(166) scale(-.44,.44)"
      />
      <Leaf
        defs={id}
        className="nocturne-sprout"
        style={{ "--nocturne-d": "1.3s" } as CSSProperties}
        transform="translate(74,37.5) rotate(122) scale(.44)"
      />
      <Leaf
        defs={id}
        className="nocturne-sprout"
        style={{ "--nocturne-d": "1.3s" } as CSSProperties}
        transform="translate(326,37.5) rotate(-122) scale(-.44,.44)"
      />
      <Bud
        defs={id}
        className="nocturne-sprout"
        style={{ "--nocturne-d": "1.72s" } as CSSProperties}
        transform="translate(36,30) rotate(-108) scale(.84)"
      />
      <Bud
        defs={id}
        className="nocturne-sprout"
        style={{ "--nocturne-d": "1.72s" } as CSSProperties}
        transform="translate(364,30) rotate(108) scale(-.84,.84)"
      />
...
export function Sprig({ transform, className, style }: Omit<MotifPieceProps, "defs">) {
  const leaf = (
    <>
      <path
        d="M0 0 C 4 -6, 13 -6, 18 0 C 13 6, 4 6, 0 0 Z"
        fill="none"
        stroke="currentColor"
        strokeWidth="1.2"
      />
      <path
        d="M2 0 C 7 -2, 13 -2, 16 0"
        fill="none"
        stroke="currentColor"
        strokeWidth="0.8"
        opacity="0.7"
  ...
  return (
    <g transform={transform} className={className} style={style} strokeLinecap="round">
      <path
        d="M6 50 C 12 34, 15 22, 27 13 C 35 8, 44 8, 50 5"
        fill="none"
        stroke="currentColor"
        strokeWidth="1.5"
      />
