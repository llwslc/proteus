import { Avatar as BaseAvatar } from "@base-ui/react/avatar";
import { cx } from "../cx";
import "./Avatar.css";

type Size = "sm" | "md" | "lg";
type Status = "online" | "busy" | "away" | "offline";

const STATUS_LABEL: Record<Status, string> = {
  online: "Online",
  busy: "Busy",
  away: "Away",
  offline: "Offline",
};

export interface AvatarProps extends React.ComponentProps<typeof BaseAvatar.Root> {
  size?: Size;
  status?: Status;
}

export function Avatar({
  size = "md",
  status,
  className,
  children,
  ...props
}: AvatarProps) {
  return (
    <span className={cx("nocturne-avatar", `nocturne-avatar--${size}`)}>
      <BaseAvatar.Root className={cx("nocturne-avatar__frame", className)} {...props}>
        {children}
      </BaseAvatar.Root>
      {status ? (
        <span
          className={cx("nocturne-avatar__status", `nocturne-avatar__status--${status}`)}
          role="img"
          aria-label={STATUS_LABEL[status]}
        />
      ) : null}
    </span>
  );
}

export function AvatarImage({
  className,
  ...props
}: React.ComponentProps<typeof BaseAvatar.Image>) {
  return <BaseAvatar.Image className={cx("nocturne-avatar__img", className)} {...props} />;
}

export function AvatarFallback({
  className,
  ...props
}: React.ComponentProps<typeof BaseAvatar.Fallback>) {
  return (
    <BaseAvatar.Fallback
      className={cx("nocturne-avatar__fallback", className)}
      {...props}
    />
  );
}
