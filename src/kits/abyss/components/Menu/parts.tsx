...
export function MenuSeparator() {
  ...
  return (
    <Separator className="abyss-separator abyss-separator--horizontal abyss-menu__separator" />
  );
}
...
export function MenuSub({ label, icon, disabled, children }: MenuSubProps) {
  ...
  return (
    <SubmenuRoot>
      <SubmenuTrigger className="abyss-menu__item" disabled={disabled}>
        <span className="abyss-menu__icon">{icon}</span>
      ...
      <Portal>
        <Positioner
          ...
          sideOffset={8}
        >
          <Popup className="abyss-surface abyss-anim-pop abyss-menu__popup">
            {children}
          </Popup>
        </Positioner>
      </Portal>
