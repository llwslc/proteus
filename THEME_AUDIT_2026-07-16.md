...
> **→ 2026-07-17 已修。**六套 demo：Select/NumberField caption 改 `label htmlFor`、Combobox/Autocomplete/OtpField 传 `label`、裸 Input 加 `aria-label`；wrapper：OtpField 新增 `label`（root `role="group"`）、Slider `label` 经 `getAriaLabel` 落 thumb 输入。kit-a11y 门禁 A 类逐控件实测全绿。
...
> **→ 2026-07-17 已修。**六套 Trigger `tabIndex=0`（禁用 `-1`）+ Shift+F10/Menu 键在区中央合成 contextmenu 开菜单，六套 zone 各配本套语汇的 focus-visible；components.md ContextMenu 条款回写；kit-a11y 门禁 B 类真键盘实测（开/Escape 关）全绿。
...
> **→ 2026-07-17 已修。**六套改 `href="#toolbar"`（自身 section 锚点）。
...
> **→ 2026-07-17 清理完成。**残留 12 处死 `cursor: not-allowed`（全在 hanabi）逐条核实均在禁用规则内后删除。
...
> **→ 2026-07-17 ④ 已修。**brass→"Loading BRASS"，同批把 bauhaus/riot/hanabi 的裸 "Loading" 一并补主题名（审计漏报的同类）。
...
> **→ 2026-07-17 ①② 已修。**Popover close 补 `aria-label="Close"`（同本套 Dialog 用词）；Avatar status 按其余四套模式补 `role="img"` + STATUS_LABEL 映射文案。kit-a11y C/D 类实测全绿。
...
> **→ 2026-07-17 已修。**同 PRISM 同批：close 补 `aria-label="Close"`，status 补 `role="img"` + 映射文案。
...
> **→ 2026-07-17 已修。**只有标了 `data-combo="confirm"` 的确认动作（出撃/消去/初期化/適用）+10，取消/关闭回落 +1；FEVER 金化改 600ms 一拍独立计时；「任何指针按下 +1」同笔对齐钉文（拆掉 interactive 过滤）。
...
> **→ 2026-07-17 已修（取修码方向）。**drop-shadow 滤镜删除，改 SVG 内偏移复制形状（`translate(2.25 2.25)` 同色描边，视觉不变、机制合规）；`NOW LOADING` 文字 `#8fa6d4`→`#4e6390`（对 loader 底 5.39:1、对白 5.98:1）。
...
> **→ 2026-07-17 混排已修。**「韧性ゲージ、破碎」→「靭性ゲージ、破砕」；全文简体字扫描无其余残留。**
