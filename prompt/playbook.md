...
6. **验收门**（依次过）——
   ...
   - **kit-distinct** 退出 0（视觉层原生、非换皮；FAIL 会指出哪些文件是照搬的 —— 别降阈值，去原生重写）。它只比 CSS 属性结构，看不到装饰构图，须另行人工核对。
   - **kit-states** 每交互态、每浮层开态渲染正常。
   - **screenshot** 逐区人工过对比度、主题个性与装饰位置，迭代打磨。
...
- **装饰构图原生**：招牌装饰 —— 角饰、标记图记、标题前缀、逐项小图标 —— 按本主题自己的母题与位置布置，不照搬 `<ref>` 的装饰槽位（同位置换张图标即换皮，kit-distinct 看不出）。装饰 `.tsx` 元素不算 Base UI 接线。把装饰收进少数原生招牌，逐槽小图记删去；语义图记如 Alert、Toast tone 保留。
