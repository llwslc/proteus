...
5. **写招牌 `.tsx` + 演示** —— 招牌件带主题专属 SVG 或结构：Switch、Loader、hero 主视觉、Toast 与模态图记、Checkbox 与 Radio 标记、占位图标。`App.tsx` 按 `app.md` 结构 + 本主题文案写，内含一份 `SECTIONS` 数组（按 `app.md` 面板清单、各 kit 同构）驱动侧栏索引与区块顺序；`registry.ts` 追加 `{ id, label, tag, app, loader }`；`theme/README.md` 移入「已建」并加链接。
6. **验收门**（依次过）——
