...
- menu：三个 Menu 并排一行、随宽换行，trigger 均为 `ghost` Button（内联标签 + 翻转 chevron）——① 12 个 MenuItem + 1 个 MenuSeparator，全部带图标，前 3 项带快捷键，第 4 项 `disabled`，分隔后末项 `tone="danger"` 且带快捷键；② 1 个 MenuRadioGroup，3 个 MenuRadioItem，`defaultValue` 取首个；③ 3 个 MenuCheckboxItem，前两个 `defaultChecked`。
