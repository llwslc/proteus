...
- menu：trigger = `ghost` Button（内联标签 + 翻转 chevron）；12 个 MenuItem + 1 个 MenuSeparator，全部带图标，前 3 项带快捷键，第 4 项 `disabled`，分隔后末项 `tone="danger"` 且带快捷键；末项之前另有两组——2 个 MenuCheckboxItem（首个 `defaultChecked`）与 1 个 MenuRadioGroup（2 个 MenuRadioItem，`defaultValue` 取首个），两组各由 MenuSeparator 分隔。
