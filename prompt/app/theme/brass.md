...
- hero：eyebrow `Pressure Console`；标题 `A **clockwork** interface kit / machined from brass`；描述关键词 brushed-brass bezels、riveted plates、gauge instruments、steam-lit motion；单位词 `Instruments`、`Token File`。
...
- hero：右侧压力表盘——表圈 + 珐琅盘面 + 刻度 11 根（末 3 根转 `danger` 红区）+ 摆针（带灯晕）往复摆（`ease-detent`）+ 盘下一枚小齿轮 `dur-spin` 自转。
- Loader：两枚啮合齿轮反向咬合转 + `Pressurizing` 字行呼吸；台高 `180`，循环播放。
