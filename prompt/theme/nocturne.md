...
- 代号 **NOCTURNE**（夜曲），取维多利亚暗夜温室的《颠茄夜园》图版册：深茄紫黑的丝绒衬底上，黄铜发丝线压出双线画框，酒红丝绒作填充，骨白衬线字押着宽字距；缠枝蔓叶自框角探入，花朵是它的活动部件——开关花开、单选月相、滑杆端花随值绽。每个控件都是一页夜册图版，也是一株夜里睁眼的花。
...
- 字体：display 与正文同族 **Cormorant Garamond**（拉丁）+ **Noto Serif SC**（汉字），display 档靠字距与字重区分；mono 槽用打字机体 **Courier Prime**（缩码、数值、时钟）；另备两支手写：`font-script` **Tangerine**（拉丁花体引言）、`font-hand` **Ma Shan Zheng**（汉文手记）。
...
- 抬升：锚定浮层挂 `shadow-pop` + 幽辉，模态挂 `shadow-modal`，面内图版框挂 `shadow-frame`。
- 浮层连接件（Arrow）是一枚 45° 菱形棱尖，面色随弹层、朝外两边压 `gilt-50` 线。
- 母题：缠枝蔓叶 sprig（框角饰）、五瓣夜花 bloom（活动部件与图记）、满月圆点（选中指示）、酒红题匾 plaque（题名牌）、黄铜发丝线（分隔），靠输入变量换色。
