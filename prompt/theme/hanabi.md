...
> 本套风格：视觉 DNA——身份、调色、字体、几何、氛围、动效语言。`<kit>` = `hanabi`。
...
- 字体：display（海报粗黑）用 **Dela Gothic One**（日文全字形），正文用 **Noto Sans JP**，数据刻度／注记（mono 槽）用像素体 **DotGothic16**。展示体只用于字形安全的串（拉丁、假名、日文形汉字），且和文串不小于 `fs-14`——更小号的和文标签、题牌走正文 `fw-700`／`fw-900`。DotGothic16 的拉丁/数字字形偏窄，凡走 mono 的拉丁/数字（数值、`// XXX` 屏码、空态英文、caption 等）一律加 `ls-2` 防粘连；纯日文 mono（如右键投放区）不需要。
