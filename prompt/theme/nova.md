...
- 对比：强调色当正文、焦点用亮档、不设深档；`text-mute` 提亮达标；青 tint 半透明面板的次要字归填充轴。
...
- 时长 `dur .22s / -slow .45s`，扫掠类另有 `dur-sweep 1.5s`、`dur-stripe .9s`；缓动 `ease (0.2, 0.8, 0.2, 1)` 快进缓出、干脆，`ease-soft (0.38, 0.1, 0.2, 1)` 给大位移起步缓，`ease-out (0.16, 1, 0.3, 1)` 给浮层。
