...
- 对比：强调色当正文、焦点用亮档、不设深档；`text-mute` 提亮达标；黄铜浮雕钮的压凹字归填充反色前景。
...
- 描边走双层黄铜 bezel 原语 `.brass-plate`：外层背景是 `bezel` 拉丝黄铜渐变 + radius，内缩 `--brass-plate-edge`（默认 `2px`）的 `::before` 是板的填充 + 内嵌 `bevel`（上沿亮、下沿暗的金属内沿）——环画在盒内那一圈，组合它的控件照常写自己的内衬与尺寸；输入变量是 `--brass-plate-fill / -bezel / -round / -bevel / -edge`。bezel 分三档：`bezel-dim` 给页内静止态的陈旧铜，`bezel` 是默认，`bezel-strong` 给浮层和升起态的亮铜。
