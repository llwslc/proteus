...
- Panel：丝绒图版——`velvet` 面 + 1px `gilt-50` 框 + 发丝内衬线 + `shadow-frame` + 四角 sprig 角饰垫于内容之下（prop `corners` 默认开）；title 落在骑顶缘居中的酒红题匾（母题 plaque，`translate(-50%,-50%)` 半出骑线），顶衬 52px 容其下摆；嵌套 Panel 无 sprig 无题匾，title 行内 `h3`。
