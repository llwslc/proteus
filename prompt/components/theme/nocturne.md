...
- Panel：丝绒图版——外框顶带 16px 把题匾出血含在盒内，内层 section 承 `velvet` 面 + 1px `gilt-50` 框 + 发丝内衬线 + `shadow-frame`、框衬 52px，四角 sprig 角饰垫于内容之下（prop `corners` 默认开）；title 落在骑框线居中的酒红题匾（母题 plaque，`translate(-50%,-50%)`）；嵌套 Panel 无 sprig 无题匾，title 行内 `h3`。
