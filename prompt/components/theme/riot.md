...
- 列表记号笔：Select、Combobox、Autocomplete、Menu 列表项悬停或键盘高亮 = 一道荧光记号笔涂过文字，全列表无划线。半透亮荧光黄、SVG 颗粒纹理、笔迹微歪软边；左端更饱和、探出首字母之前，带高于文字；实色盖满整词、右端提笔渐隐一道短尾，各控件长短词一致。选中项文字恒荧光 `primary`（Anton 大写加勾）不变，被高亮时同样盖记号笔。换色走 `--riot-marker-color`。
