...
- 悬停：分段控件和触发条用 `tint` 荧光 wash 纯底；带硬影的按钮整块上浮一步、硬影随之加深（press 的反向），ghost 按钮盖 `tint` wash、文字转 `text-bright`；图标钮和动作按钮的文字转荧光 `primary`，菜单触发器转 `text-bright` 纯黑；列表项盖一道荧光记号笔涂划；字段族悬停不变，整框只在聚焦或打开时点亮。
...
- Checkbox：`ink` 粗黑方框 + 硬偏移影；勾选 = 荧光 `secondary` 粉的手画叉 `✗`，indeterminate = 一道粗横杠；勾上的标签盖一道荧光橙删除线；行标签走 `font-tag` 注记体（`.riot-tag`，`fs-18`），像涂鸦标语。
...
- Combobox：列表项皮肤同 Select；多选芯片 = `primary` 实填 + `on-fill` 字，字体同字段；高亮转 `secondary`。
...
- Panel：一张斜钉的撕纸剪报（`--riot-tilt` 微旋转）——斜的是纸面本身，胶带与订书钉压在纸上层；一角贴胶带或订书钉。
- 占位图标是粗糙的手绘线形。
