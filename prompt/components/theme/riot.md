...
- 列表记号笔：Select、Combobox、Autocomplete、Menu 列表项悬停或键盘高亮 = 一道荧光记号笔涂过文字。半透亮荧光黄、SVG 颗粒纹理、笔迹微歪软边；左端更饱和、探出首字母之前，带高于文字；横扫约多半行宽，两端（左圆头、右渐隐尾）定宽不拉伸、只中段随宽伸缩；可见实色约半、右端提笔渐隐。选中项文字恒荧光 `primary`、加勾，不变，被高亮时同样盖记号笔。换色走 `--riot-marker-color`。
