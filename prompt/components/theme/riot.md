...
- 列表记号笔：Select、Combobox、Autocomplete、Menu 列表项悬停或键盘高亮 = 一道荧光记号笔涂过文字（带前置图标的菜单项连图标一并盖住）。半透亮荧光黄、SVG 颗粒纹理、笔迹微歪软边；左端更饱和、探出首字母之前，带高于文字；横扫约多半行宽、可见实色约半、其后提笔渐隐。选中项文字恒荧光 `primary`、加勾，不变，被高亮时同样盖记号笔。换色走 `--riot-marker-color`。
