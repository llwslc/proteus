...
- 悬停：分段控件和触发条用 `tint-soft` 纯底，其中 Tabs、NavMenu 用 `180deg transparent→tint-soft` 渐变 + 下划线；图标钮和动作按钮的文字转 `primary`；实填按钮（primary／secondary／danger）填充提亮一档（brightness），ghost 盖 `ghost-hover` 底、文字转 `primary`。
