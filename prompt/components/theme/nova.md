...
- 悬停：分段控件和触发条用 `tint-soft` 纯底，其中 Tabs、NavMenu 用 `180deg transparent→tint-soft` 渐变 + 下划线；图标钮和动作按钮的文字转 `primary`；实填按钮（primary／secondary／danger）填充提亮一档（brightness），ghost 盖 `ghost-hover` 底、文字转 `primary`；字段整框悬停边框升 `line-strong`，一体变化。
- 焦点：布尔开关（Checkbox、Switch、Radio）用 `glow-focus`；分段控件和触发条用 `inset 0 0 0 1px line-strong`；输入框聚焦时边框点亮成 `primary` + 加 `glow-focus`。
- 危险态走 danger 家族：`-fill / -wash / -highlight / -text / -inset`；危险行高亮：盖 `danger-wash`、前景转 `danger-text`。
