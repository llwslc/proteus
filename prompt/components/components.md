...
- **悬停**：分段控件、触发条统一用柔色纯底；图标钮、动作按钮的文字转主色，菜单触发器、列表项转亮色文字；列表项与菜单项行内的前导图标在高亮态取与该行文字相同的色，静止态可暗一档；普通按钮必有可见的悬停反馈，**形式由 theme 定**；选中态、开启态要压过悬停态（悬停的「禁用守卫」用 `:where()` 包住、不抬权重）。
