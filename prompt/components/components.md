...
- **Progress**：props `label·showValue`；结构 `Root > head[label + Value 在右] + Track > Indicator`，Indicator 从左满宽推进。`value` 为 null 即**不定态**（Base UI 标 `data-indeterminate`、不显数值、不挂确定态的推进端记号）：Indicator 收成沿轨**连续单向扫掠**的段——匀速前行、无缝循环、不停顿不回弹，与「从左满宽的定值」一眼可分；扫掠段的宽度与填充皮由 theme 定，运动骨架跨 kit 同构。
