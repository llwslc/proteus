...
> **文字对比**：读内容的功能字（`-mute`、说明、错误）默认在其栖身表面 ≥4.5:1，焦点描边／状态图标 ≥3:1。强调色当正文／焦点走哪一档由各 theme 按自己的身份定，两条都成立、无高下：留满饱和的招牌色（响度优先），或取本家族更深一档、提正文对比。各 theme 在自己文档写一条对比取舍即可。纯装饰字、商标字、禁用态不在此列；填充与强调色底上的反色前景取该 theme 的反色前景档，对渲染像素 ≥3:1 即可，不为凑更高档加深招牌色。
