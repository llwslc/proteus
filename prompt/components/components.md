...
- **Button**：变体 `primary·secondary·danger·ghost·icon·icon-ghost`（默认 primary）；尺寸 `sm·md·lg`（默认 md）；props `variant·size·icon·tilt`；`tilt` 布尔＝把按钮微旋斜钉在版面上（角度由 theme 定），摁下回正；`forwardRef`，可当触发器。图标在文字前、和文字在同一个 inline-flex 里居中。icon-ghost 透明、无底无边、静止时不露 frame。
