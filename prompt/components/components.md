...
- **Combobox**：props `items·placeholder·emptyText·label·side`（默认 bottom）`·align`（默认 center）·`multiple`；结构 `InputGroup[左图标? + Input flex:1 + Clear + Trigger(chevron)] + Popup[Empty + List > Item(带选中指示)]`；`multiple` 时 Input 位换成 `Chips[已选各一 Chip(标签 + 移除钮) + Input 续在末尾]`——芯片是纯填色标签、字号同该套字段输入、随行换行，移除记号比标签安静一档、达 §3 的状态图标档；续写的输入与芯片同高，排不下才另起一行；不选时框高同该套单选字段，每换一行按行高涨。**芯片的填色与形状由 theme 定、各 kit 钉在皮文档。**弹层向下、开在 `6px` 处、宽度随 `--anchor-width`；过滤走 Base UI 内建；`items` 项收字符串或 `{ label, disabled }`，label 归一成串传 Root，禁用落在 `Item`。
