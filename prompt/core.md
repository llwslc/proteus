...
- **唯一断点 `768px`**，不另设别的断点。`≤768` 组件走手机态——Tabs、NavMenu **横滚**不换行；横滚条不隐藏、显示一条标准细条（`scrollbar-width: auto` + `::-webkit-scrollbar` 细条，给桌面一个可拖的可滚提示；macOS 叠加条仍随系统设置）；Toolbar **换行**不横滚。外壳、侧栏、演示页响应式 → `app.md`。
