...
- **NavigationMenu**：props `items·onLinkClick`；`List > Item[Trigger(chevron 开转 180°) + Content > grid > Link]`；桌面两列，列宽走 src/shared 的 `--shell-navmenu-col-w`（各 kit 同值、kit 内 alias 成 `--<kit>-navmenu-col-w`），网格 `repeat(2, minmax(var(--<kit>-navmenu-col-w), 1fr))`。
