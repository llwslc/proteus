...
- 内容宽控件（按钮、图标按钮）放进纵向 flex 容器（面板体、表单、字段栈）时，加 `align-self: flex-start` 收成内容宽，别被默认 `align-items: stretch` 拉满整行成一条整宽块；整行通栏只留给 Input、Select、textarea、Accordion 这类输入控件。
