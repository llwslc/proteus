...
- 五个强调色家族，只有 warning 配一个 `-deep` 暗档：primary 群青蓝 `#1a4fd6`，secondary 墨黑 `#1b1b1b`，success 草绿 `#2f9e44`，warning 原黄 `#f5b612 / #a8760a`，danger 朱红 `#e23121`。
...
- AlertDialog 按 `tone` 重染，tone 取 danger／warning／primary：border 仍黑，标题和图标随 tone 变；表面顶部加一条 tone 的**实色 bar**（不是径向渐变），几何同 Dialog。
- Panel：角部放三原形角标 + meta；Toast：锚在右下角、向上堆叠，新的在最前（最靠角）、旧的在后面露出一截，左缘一条 tone 实色 bar + 一个三原形的 tone 图标；Dialog 标题配一个三原形图标。
...
- 共享配方的颜色就近覆盖：`--bauhaus-bar-color / -marker-color / -title-color`。
