...
- AlertDialog 按 `tone` 重染，tone 取 danger／warning／primary：border 仍黑；**tone 做成顶部一整条实色色带**（是一块色域、不是细线），标题和一个反白的三原形 tone 图记压在色带上（黄色带上的字用 `on-warning`，其余用 `on-fill`）；描述、body、actions 在下面的纸面上。
