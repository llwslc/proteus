...
**B. 对比度铁律**
3. **半透明填充别盖在有色边框上** —— 「边框色打底 + `::before` 填充」时,若 `::before` 在某状态变成接近透明,底下的亮色边框会透上来铺满整块,前景(文字/滑块)直接看不清。这些状态的填充必须**深色不透明**。⚠️ **改一个控件,就要把它的所有状态/变体一起核对**(别只改一个):输入框聚焦、ghost 按钮、switch 的 **on 和 off 各自**都栽过 —— on 态曾是「青滑块压在青轨道上」消失,解法是轨道做「左青右暗」,让滑块落在暗处对比够。
