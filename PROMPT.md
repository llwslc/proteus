...
- **浮层**:Tooltip(**无箭头的纯切角浮层**,和 Select/Dialog 浮层一致)、Dialog(`DialogClose` **复用 Button**,经 `render`)、Drawer(**边缘锚定的 Dialog**,从屏幕边缘滑入的侧栏)、Toast(Provider + `useToast`)
...
2. **`clip-path` 会裁掉所有子元素/伪元素** —— 凡是要触达边缘或角的东西,要么挪到不被切的位置,要么别让它探出:
   ...
   - Tooltip **别用箭头**(会被 popup 切角裁成缺口)→ 无箭头的**纯切角浮层**(和 Select/Dialog 浮层一致;别加装饰短线之类的小料,会像故障);
