...
description: 盒外绘制不被裁门——两查：① 模态与抽屉里凡 overflow 非 visible 的容器，都不得裁掉它承载内容的盒外绘制（焦点环、骑在边界上的旋钮与花记）；② 滑块旋钮不得被任何祖先裁切——`clip-path` 会连子元素一起裁，容器的切角形状必须挪到 `::before` 承载（nova 晶块曾只剩 44% 可见、辉光与焦点环全没）。改滑块旋钮尺寸、控件焦点环、容器切角形状、模态体内衬后跑。
---
