...
- **Raw size in .css** — a component/overlay `width`/`height`/`min-`/`max-` written as a literal instead of a `--<kit>-<组件>-<角色>` token. Exemptions follow components.md §3 verbatim: `≤8px` sub-grid values and contextual forms (`clamp`/`calc`/`%`/`dvh`/`em`, Base UI anchor vars). `max()`/`min()` are NOT exempt — their literal operands are still hand-written sizes.
...
- **Duplicate selector** — the same selector written twice in one file under the same at-rule context: the second block was appended instead of merged, so one element's properties sit in two places and can silently override each other (a `padding` shorthand undone by a later `padding-left`). Exempt: a later rule declaring ONLY `animation-*`, since each App.css groups its whole entrance layer in a trailing section. Fix = fold the second rule's declarations into the first, in place.
