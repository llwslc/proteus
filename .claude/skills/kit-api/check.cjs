...
function parse(k, c) {
  ...
  while ((m = re.exec(s))) {
    const props = new Set();
    for (const line of m[3].split('\n')) { const pm = line.match(/^\s*(\w+)\??:/); if (pm) props.add(pm[1]); }
...
for (const c of comps) {
  ...
  if (withIface.length && withIface.length < present.length) {
    ...
    const union = new Set();
    withIface.forEach((k) => pk[k].ifaces[mi].props.forEach((p) => union.add(p)));
    const diff = [...union].filter((p) => new Set(withIface.map((k) => pk[k].ifaces[mi].props.has(p))).size > 1);
    if (diff.length) { out.push(`FAIL ${c}: ${mi} prop(s) not in every kit — ${diff.join(', ')}  [${withIface.map((k) => `${k}:{${[...pk[k].ifaces[mi].props].join(',')}}`).join('  ')}]`); fails++; }
