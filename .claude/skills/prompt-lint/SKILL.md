...
- **Invented / calque words** — coined compounds that aren't native to the spec's language. In Chinese: 翻深 (write 加深 / 转深色), 翻 as "switch to" (write 转), and similar literal calques. A defined bracketed term-of-art (「点亮表面」) reused consistently is fine; an ad-hoc non-word is not. Read aloud — if a native speaker wouldn't say it, flag it.
...
- **Parenthesis overload** — prose where most lines carry a `(...)` aside reads like footnotes, not documentation. A parenthetical is fine for a bare value/example right after its term; it is NOT fine as the default container for clauses. Rough smell: more than ~1 in 3 lines parenthesized, or any line with ≥3. Re-flow asides into colons, em-dashes 「——」, comma clauses, or separate sentences.
  - **Bold is systematic, not sprinkled.** `**bold**` is reserved for: a definition-list lead applied to EVERY sibling bullet (core §3/§5/§6 style); a set of names emphasized uniformly (all font names, the kit codename, the cap-class) and consistently across sibling kits; a cross-kit-parallel signature keyword; or a critical inline negation/constraint. Flag a one-off bold on a single narrative bullet whose peer bullets are plain (e.g. only `**Switch 是一只眼**` of five §7 bullets), and a bold one kit applies that its sibling kit omits for the same item (e.g. abyss bolding palette-family names while nova doesn't).
