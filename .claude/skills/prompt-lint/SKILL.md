...
- **How, not why.** Each line states a decision: what to do, what value, what to name, what not to do. Cut anything that doesn't change the output — rationale ("because / to avoid / otherwise"), history ("was X now Y, originally, accepted/rejected"), incident anecdotes ("this broke, the old bug"), framework restatement (re-explaining an API / CSS property / library the model knows), and hedging ("note that, for clarity"). **Negating a past misconception is still residue** — "not an X thing / not actually Y" corrects a belief the reader never held; state the rule positively and don't name the wrong idea at all. **Absence written as a patch-note is the same residue** — a single part described by what it lacks (「不带投影」「不再 X」「去掉了 X」) records the edit, not the design; describe the resulting form positively (「平贴」). A voice-level exclusion LIST stays (「无渐变、无辉光」 defines the register).
- **Cite the code literal.** A clause claiming a value names the token/scope exactly as the code defines it — never an invented shorthand that renames or widens the claim (a per-family statement for a token only one family has is a WRONG SPEC, not a style nit). Back-writing a live design cites the CODE's literals, never a paraphrase of the chat that converged on it.
...
- **Siblings are the control.** In parallel per-variant docs (the `theme/<kit>.md` set), a clause one variant states that its siblings all omit is a candidate for cruft — the siblings prove it unnecessary: a redundant restatement of an earlier section, a rule a shared layer already owns, or a note the others plainly didn't need. Cut it unless it's a genuine variant-unique trait. `sh .claude/skills/prompt-lint/parallel.sh` dumps every section with all kits adjacent so the outlier bullet shows at a glance.
...
- **Native words.** No coined compounds or literal calques from another language. A defined term-of-art reused consistently is fine; an ad-hoc non-word is not. **Terse means fewer words, not crushed words** — keep a grammatical skeleton (a verb/connective); a clause that piles nouns with no connective (`四边全驱动定位`), re-segments wrong (`等长写`), elides the subject/relation past recovery (`带高于行`), or collides with a real word (`高档`) fails even if every morpheme is native. Read it aloud — if a fluent speaker wouldn't say it, or must back up to re-parse, flag it. **Over-compression is the same defect as fluff** — a fresh session must re-derive the decision from the line alone; never fix verbosity by minting opacity, or vice versa.
