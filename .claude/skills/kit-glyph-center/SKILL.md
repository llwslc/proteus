...
Two non-obvious requirements baked in, both learned from misses: measure **ink not box** (box-center hid the prism sigil offset), and measure **at rest** (the abyss drawer key reads −2.7px mid-animation but 0 at rest). Proven fail-on-broken: against the pre-fix prism sigil it flags all three modal titles at −3.5px.
