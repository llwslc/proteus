...
It normalizes the **skin** out of each component CSS + `theme/effects.css` — token refs → `var(T)`, colors → `#H`/`C`, sizes → `N`, `url(#…-edge)` → `url(#F)`, the kit prefix stripped — leaving only STRUCTURE (selectors, property sets, recipe shape). Then it diffs that structure file-by-file against every sibling and reports two numbers:
...
The acceptance trio: **kit-lint** (static token contract) · **kit-states** (rendered dynamic states) · **kit-distinct** (the visual layer is native, not a reskin).
