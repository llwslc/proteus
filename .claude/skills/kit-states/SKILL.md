...
When the kit gains an interaction state the sweep doesn't cover, add it to `states.cjs` and to its `EXPECT` list so a miss is reported — the gate is only as good as its state list. (The kit *list* needs no maintenance — it comes from the `src/kits/` scan.)
